...
def read_image_cv2(file_name, format=None):
    """# NOTE modified from detectron2, use cv2 instead of PIL
...
def select_point(shape, poly, gt_mask, cfg):
...
def add_small_obj(img, gt_mask, poly, box, polys_gt, cfg):
    poly_img, mask = get_mask_img(img, poly)
    x, y = select_point(img.shape, poly.copy(), gt_mask, cfg=cfg)
    if x == -1:
        box = []
        return img, poly, box
...
def small_aug(img, poly, box, label, num, cfg):
    N = len(poly)
    ...
    for i in range(N):
        ...
        if poly[i][0]['area'] < 32*32:
            for k in range(num):
                img, poly_s, box_s, gt_mask = add_small_obj(img, gt_mask, poly[i][0].copy(), box[i].copy(), poly, cfg=cfg)
                if len(box_s) == 0:
                    continue
                poly.append([poly_s])
