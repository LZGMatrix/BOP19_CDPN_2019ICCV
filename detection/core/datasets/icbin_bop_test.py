    ...
    def __call__(self):
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                str_im_id = str(im_id)
                scene_root = osp.join(self.dataset_root, f"{scene_id:06d}")
                im_path = osp.join(scene_root, "rgb/{:06d}.png").format(im_id)
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno['obj_id']
                    if ref.icbin.id2obj[obj_id] not in self.select_objs:
                        continue
...
def get_icbin_metadata(obj_names):
...
for obj in ref.icbin.objects:
    name = "icbin_bop_{}_test".format(obj)
    select_objs = [obj]
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    MetadataCatalog.get(name).set(
        ...
        **get_icbin_metadata(obj_names=used_cfg["objs"]),
    )
