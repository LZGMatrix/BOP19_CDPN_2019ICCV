...
class HB_PBR_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get('dataset_root', osp.join(DATASETS_ROOT, "BOP_DATASETS/hb/train_pbr"))
...
SPLITS_HB_PBR = dict(
    hb_pbr_train=dict(
        name="hb_pbr_train",
        objs=ref.hb.objects,  # selected 16 objects
...
for obj in ref.hb.objects:
    for split in ["train"]:
        name = "hb_pbr_{}_{}".format(obj, split)
        if split in ["train"]:
            filter_invalid = True
        elif split in ["test"]:
            filter_invalid = False
