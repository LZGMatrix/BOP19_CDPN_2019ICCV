...
class LMO_BOP_TEST_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/lm/test
        ...
        if True:
            ...
            for scene_id, im_id in tqdm(scene_im_ids):
                ...
                for anno_i, anno in enumerate(gt_dicts[scene_id][str_im_id]):
                    obj_id = anno['obj_id']
                    if ref.lm_full.id2obj[obj_id] not in self.select_objs:
                        continue
...
SPLITS_LM = dict(
    ...
    lmo_test=dict(
        ...
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo/models"),
        objs=LM_OCC_OBJECTS,
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo/test_targets_all.json"),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
        height=480,
...
for obj in ref.lmo_full.objects:
    name = "lmo_bop_{}_test".format(obj)
    select_objs = [obj]
