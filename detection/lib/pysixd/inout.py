...
def load_cam_params(path):
    if '.json' in path:
        return load_cam_params_json(path)
    with open(path, "r") as f:
    ...
    with open(path, "r") as f:
        ...
        if not res["ests"] or res["ests"] == [{}]:
            ...
            for est in res["ests"]:
                ...
                if isinstance(est["score"], six.string_types):
                    if "nan" in est["score"]:
                        est["score"] = 0.0
                    else:
    ...
    while True:
        ...
        if line.startswith('comment TextureFile'):
            ...
            if elems[-1] == "vertex_indices"  or elems[-1] == 'vertex_index':
                ...
                logger.warn("Warning: Not supported face property: " + elems[-1])
