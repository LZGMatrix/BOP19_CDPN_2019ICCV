...
def pnp_ransac_custom(image_points, model_points, K,
        pnp_type=cv2.SOLVEPNP_ITERATIVE, ransac_iter=100, ransac_min_iter=10,
        ransac_reprojErr=2):
    ...
    for iter in range(ransac_iter):
        ...
        if (err_mean < best_err or inliers_num > best_inliers_num) and inliers_num >= 4:
            ...
            success, R_exp, T_vector = cv2.solvePnP(
                    inliers_model_pts, inliers_image_pts,
                    K, dist_coeffs,
                    flags=pnp_type)
            # compute reprojection error
            pts_2d, _ = cv2.projectPoints(model_points, R_exp, T_vector, K, dist_coeffs)
...
def pnp_v2(
    points_3d, points_2d, camera_matrix, method=cv2.SOLVEPNP_EPNP, ransac=False, ransac_reprojErr=6.3, ransac_iter=100
):
    """
...
def calc_emb_bp(depth, R, T, K):
    ...
    for y in range(height):
        for x in range(width):
            if mask[y, x] == 0:
                continue
            p2d = np.array([x, y, 1]).reshape((3, 1))
            p3d = np.squeeze(np.matmul(R.T, depth[y, x] * np.matmul(Kinv, p2d) - T.reshape((3, 1))))
            ProjEmb[y, x] = p3d

    # print('ProjEmb bp: min {} max {}'.format(ProjEmb.min(), ProjEmb.max()))
    return ProjEmb
...
def calc_emb_bp_fast(depth, R, T, K):
    ...
    ProjEmb = np.einsum(
        "ijkl,ijlm->ijkm", R.T.reshape(1,1,3,3),
        depth.reshape(height, width, 1, 1) * np.einsum("ijkl,ijlm->ijkm", Kinv.reshape(1,1,3,3), grid_2d.reshape(height, width, 3, 1)) -
        T.reshape(1, 1, 3, 1)).squeeze() * mask.reshape(height, width, 1)
...
def calc_emb_bp_torch(depth, R, T, K, ProjEmb):
    ...
    # [H,W,3]
    ProjEmb.data = torch.matmul(Rinv_expand,
                                depth.view(height, width, 1, 1) * torch.matmul(Kinv_expand, grid_2d.view(height, width, 3, 1)) -
                                T_expand).squeeze() * mask.view(height, width, 1)
...
def calc_pts_diameter(pts):
    """Calculates diameter of a set of points (i.e. the maximum distance
