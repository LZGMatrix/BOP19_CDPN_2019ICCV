...
def iiprint(*args, **kwargs):
    ...
    if True:
        ...
        date = colored(
            "[{}@{}:{}]".format(get_time_str("%m%d_%H%M%S"), filename, caller.lineno),
            "yellow",
        )
...
def ddprint(*args, **kwargs):
    ...
    if True:
        ...
        print(
            date + ' ' + colored("DBG ", "yellow", attrs=["blink"]) + " ".join(map(str, args)),
            **kwargs,
        )
...
def send_email(subject, body, to, smtp_server="localhost", port=1025):
    # https://github.com/facebookresearch/Detectron/blob/master/detectron/utils/logging.py
